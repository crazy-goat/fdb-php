...
/**
 * Unit tests for the `fdb_future_get_uint64` accessor bound in issue #94
 * (via `FutureUInt64::await()`), which backs
 * `Database::getServerProtocol()`.
 *
 * As with FutureDoubleTest, no FoundationDB cluster or real client library
 * is needed: a tiny stub C library is compiled on the fly and wired into a
 * constructor-less NativeClient via reflection, exercising the real
 * production Future paths end-to-end.
 */
final class FutureUInt64Test extends TestCase
{
    ...
    #[Test]
    public function awaitReturnsTheValueResolvedByFdbFutureGetUint64(): void
    {
        $this->setStubValue(0);
        self::assertSame(0, $this->makeFuture()->await());

        $this->setStubValue(0x00F08044D0000001);
        self::assertSame(0x00F08044D0000001, $this->makeFuture()->await());
    }

    #[Test]
    public function awaitCachesTheResultAcrossCalls(): void
    {
        $this->setStubValue(4500);
        $future = $this->makeFuture();

        self::assertSame(4500, $future->await());
    ...
    private static function buildStub(): FFI
    {
        $source = <<<'C'
            ...
            static unsigned long long g_uint64_value = 0;
            ...
            void fdb_phpunit_stub_set_uint64(unsigned long long v) { g_uint64_value = v; g_released = 0; }
            C;
        ...
        $header = <<<'C'
            ...
            void fdb_phpunit_stub_set_uint64(unsigned long long v);
            C;
    ...
    private function setStubValue(int $value): void
    {
        $stub = self::$stub;
        \assert($stub instanceof FFI);
        /** @phpstan-ignore-next-line method.notFound — dynamic FFI binding to the compiled stub */
        $stub->fdb_phpunit_stub_set_uint64($value);
    }
    ...
    private function makeFuture(): FutureUInt64
    {
        $stub = self::$stub;
        \assert($stub instanceof FFI);
